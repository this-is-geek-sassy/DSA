...
public class Stock1 {
    private static int helper (int[] prices, int i, boolean holding, int[][] memory) {
        ...
        if (holding) {
            memory[a][i] = Math.max(prices[i], helper(prices, i+1, holding, memory));
        } else {
    ...
    public static int maxProfitBottomUp(int[] prices) {
        ...
        for (int i = prices.length-2; i>=0; i--) {
            ...
            memory[1][i] = Math.max(memory[1][i+1], prices[i]);
        }
        return memory[0][0];
    }
    ...
    public static int maxProfitBottomUpSpaceOptim(int[] prices) {
        ...
        for (int i = prices.length-2; i>=0; i--) {
            ...
            memory[1][k] = Math.max(memory[1][getNext(k)], prices[i]);
            k = getNext(k);
        }
    ...
    public static void main(String[] args) {
        ...
        int ans = maxProfitBottomUpSpaceOptim(prices);
